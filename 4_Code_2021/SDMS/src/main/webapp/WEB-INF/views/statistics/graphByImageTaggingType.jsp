...
<!--
	...
	$(function() {
		...
		$('select[name=useYn]').change(function() { search(); });
	 	...
	 	// Chart Data Setting
		var rectangleCnt = new Array();
		var polygonCnt = new Array();
		var labels = new Array();

		<c:forEach var="item" items="${barAllList}">
			labels.push("${item.imageFileRegistDt}");
			rectangleCnt.push(parseInt("${item.rectangleCnt}"));
			polygonCnt.push(parseInt("${item.polygonCnt}"));
		</c:forEach>
	    ...
	    var pieOptions = {
	 	...
	 	// Chart Data Setting
		var rectangleTotalCnt = 0;
		var polygonTotalCnt = 0;
		var labels = new Array();

		<c:forEach var="item" items="${pieList}">
			rectangleTotalCnt += parseInt("${item.rectangleCnt}");
			polygonTotalCnt += parseInt("${item.polygonCnt}");
		</c:forEach>
	    ...
	    var pieData = {
   	    	labels: [
   	         ...
   	         datasets: [{
                 data: [rectangleTotalCnt, polygonTotalCnt],
                 backgroundColor: [
                     "rgba(255, 0, 30, 0.5)",
                     "rgba(0, 174, 255, 0.5)"
                 ],
					...
					<table class="table table-striped table-bordered table-hover">
						...
						<thead>
							<tr>
								...
								<th><spring:message code="admin.statistics.rectangleCnt" /></th>
								<th><spring:message code="admin.statistics.polygonCnt" /></th>
						...
						<tbody>
							...
							<c:forEach var="item" items="${barList}">
								<tr>
									...
									<td class="le ellipsis" title="${item.rectangleCnt}">${item.rectangleCnt}</td>
									<td class="le ellipsis" title="${item.polygonCnt}">${item.polygonCnt}</td>
			...
			<div class="table-responsive">
				<table class="table table-striped table-bordered table-hover">
					...
					<tbody>
						...
						<c:forEach var="item" items="${pieList}">
							<tr>
								<spring:message var="rectangle" code="admin.statistics.rectangle" />
								<td title="${rectangle}">${rectangle}</td>
								<td title="${item.rectangleCnt}">${item.rectangleCnt}</td>
								<td title="${item.rectangleAvg}%">${item.rectangleAvg}%</td>
							</tr>
							<tr>
								<spring:message var="polygon" code="admin.statistics.polygon" />
								<td title="${polygon}">${polygon}</td>
								<td title="${item.polygonCnt}">${item.polygonCnt}</td>
								<td title="${item.polygonAvg}%">${item.polygonAvg}%</td>
							</tr>
							<tr class="info">
								<td title="합계">합계</td>
								<c:set var="totalAvg" value="${item.rectangleAvg+item.polygonAvg}" />
