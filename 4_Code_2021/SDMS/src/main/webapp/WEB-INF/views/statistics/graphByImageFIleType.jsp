...
<!--
	...
	$(function() {
		...
		$('select[name=useYn]').change(function() { search(); });
	 	...
	 	// Chart Data Setting
		var jpgCnt = new Array();
		var pngCnt = new Array();
		var bmpCnt =  new Array();
		var labels = new Array();

		<c:forEach var="item" items="${barAllList}">
			labels.push("${item.imageFileRegistDt}");
			jpgCnt.push(parseInt("${item.jpgCnt}"));
			pngCnt.push(parseInt("${item.pngCnt}"));
			bmpCnt.push(parseInt("${item.bmpCnt}"));
		</c:forEach>
		...
		<c:forEach var="item" items="${pieList}">
			jpgTotalCnt += parseInt("${item.jpgCnt}");
			pngTotalCnt += parseInt("${item.pngCnt}");
			bmpTotalCnt += parseInt("${item.bmpCnt}");
		</c:forEach>
	    ...
	    var pieData = {
   	    	labels: [
   	         ...
   	         datasets: [{
                 data: [jpgTotalCnt, pngTotalCnt, bmpTotalCnt],
                 backgroundColor: [
                     "rgba(255, 0, 30, 0.5)",
                     "rgba(62, 58, 126, 0.5)",
                     "rgba(0, 174, 255, 0.5)"
					...
					<table class="table table-striped table-bordered table-hover">
						...
						<thead>
							<tr>
								...
								<th><spring:message code="admin.statistics.jpgCnt" /></th>
								<th><spring:message code="admin.statistics.pngCnt" /></th>
								<th><spring:message code="admin.statistics.bmpCnt" /></th>
						...
						<tbody>
							...
							<c:forEach var="item" items="${barList}">
								<tr>
									...
									<td class="le ellipsis" title="${item.jpgCnt}">${item.jpgCnt}</td>
									<td class="le ellipsis" title="${item.pngCnt}">${item.pngCnt}</td>
									<td class="le ellipsis" title="${item.bmpCnt}">${item.bmpCnt}</td>
		...
		<div class="jarviswidget col-sm-6" id="wid-id-3" data-widget-colorbutton="false" data-widget-fullscreenbutton="false" data-widget-editbutton="false" data-widget-sortable="false">
			...
			<div class="table-responsive">
				<table class="table table-striped table-bordered table-hover">
					...
					<thead>
						<tr class="danger">
							<th><spring:message code="admin.statistics.fileType" /></th>
					...
					<tbody>
						...
						<c:forEach var="item" items="${pieList}">
							<tr>
								<spring:message var="jpg" code="admin.statistics.jpg" />
								<td title="${jpg}">${jpg}</td>
								<td title="${item.jpgCnt}">${item.jpgCnt}</td>
								<td title="${item.jpgAvg}%">${item.jpgAvg}%</td>
							</tr>
							<tr>
								<spring:message var="png" code="admin.statistics.png" />
								<td title="${png}">${png}</td>
								<td title="${item.pngCnt}">${item.pngCnt}</td>
								<td title="${item.pngAvg}%">${item.pngAvg}%</td>
							</tr>
							<tr>
								<spring:message var="bmp" code="admin.statistics.bmp" />
								<td title="${bmp}">${bmp}</td>
								<td title="${item.bmpCnt}">${item.bmpCnt}</td>
								<td title="${item.bmpAvg}%">${item.bmpAvg}%</td>
							</tr>
							<tr class="info">
								<td title="합계">합계</td>
								<c:set var="totalAvg" value="${item.jpgAvg+item.pngAvg+item.bmpAvg}" />
