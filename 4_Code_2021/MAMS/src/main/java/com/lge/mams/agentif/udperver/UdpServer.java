...
public class UdpServer extends Thread {
	...
	public void startServer(int port, SeqImageProvider prov, VoiceHandler voiceHandler) {
		this.port = port;
	...
	public void run() {
		try {
			...
			while (true) {
				...
				h.fromBytes(header);
				h.setRemoteIp(dp.getAddress() == null ? "" : dp.getAddress().toString());
...
//				if( h.getReserved() == 0 ) {
//					logger.debug("voice");
//					voiceHandler.handle(h, data);
//				}else if( h.getReserved() == 1){
//					logger.debug("image");
//					imageProvider.add(h, data);
//				}else {
//					logger.warn("unknown reserved : {}", h.getReserved());
//				}
