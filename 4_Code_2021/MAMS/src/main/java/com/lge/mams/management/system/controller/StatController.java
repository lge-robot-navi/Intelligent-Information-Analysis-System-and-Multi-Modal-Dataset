...
public class StatController extends BaseController {
	...
	private final String PREFIX = "system/stat/";


	@Autowired
	TbAgentStatRepository repoStat;
	...
	public String list(@ModelAttribute AbstractPage entity, HttpSession session, Model model) throws Exception{
		...
		Sort sort = new Sort(Sort.Direction.DESC, "statSn");
		...
		Page<TbAgentStat> result = repoStat.findByStatDtBetween(page, start, end);
...
//		
//		for(TbAgentStat info:result.getContent()) {
...
//		}
		...
		return PREFIX + "stat-list" + TilesSuffix.EMPTY;
	}
	
	@RequestMapping(method = RequestMethod.GET, value = "replay")
	public String replay(@ModelAttribute("entity") AbstractPage entity, HttpSession session, Model model) {

		if (StringUtils.isEmpty(entity.getStartDt()) && StringUtils.isEmpty(entity.getEndDt())) {
    		entity.setStartDt(DateSupportUtil.getToday());
    		entity.setEndDt(DateSupportUtil.getToday());
    	}
		logger.debug("entity:{}", ToStringBuilder.reflectionToString(entity));
		
		
		return "system/stat/stat-replay";
	}
