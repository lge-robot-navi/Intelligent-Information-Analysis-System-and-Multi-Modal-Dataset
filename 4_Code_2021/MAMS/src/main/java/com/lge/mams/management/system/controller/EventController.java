...
public class EventController extends BaseController {
	...
	public String list(@ModelAttribute AbstractPage entity, HttpSession session, Model model) throws Exception {
		...
		List<TaCodeInfo> codes = repoCode.findByCdgrpCdAndUseYn("TA011", "Y");

		for (TbEventInfo info : result.getContent()) {
			String id = info.getAbnormalId();
			// codes.stream().filter( code -> code.getCodeCd().equals(id)).forEach(code ->
