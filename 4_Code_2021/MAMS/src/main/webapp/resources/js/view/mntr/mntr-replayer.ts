...
declare var moment: typeof mmoment;
...
function createMntrPlayer() {
  var player = {
      ...
      this.timerid = setInterval(function () {
        ...
        if (player.isrequesttime(time)) {
          ...
          if (player.isfinished(time)) {
            player.stop();
            if (player.evtstop) player.evtstop();
          }
        }
    ...
    isrequesttime: function (time) {
      ...
      if (this.strrequestmin == moment(this.startmin + elapsed).format("YYYYMMDDHHmm")) return false;
      return true;
    },
    getimageurl: function (time) {
      ...
      while (true) {
        if (this.infos.length < 1) {
          if (this.evtimage) this.evtimage(elapsed + this.startmin);
        ...
        if (Math.abs(diff) < this.interval) {
          this.infos.shift();
          if (this.evtimage) this.evtimage(o.time);
