...
declare var $: typeof mjquery; /////////////////////////////////////////////////// mntrenvmap.js (환경맵 처리)
declare var appbody;
  ...
  axios
    ...
    .then(function (res) {
      ...
      reader.onload = function () {
        $("#envmapImg").attr("src", reader.result as string);
      };
    })
...
setInterval(function () {
  if (!appbody) return;
  if (!appbody.autorefresh) return;
  if (!envmapstat.mapid) return;
  if (!appbody.autorefreshinterval) return;
