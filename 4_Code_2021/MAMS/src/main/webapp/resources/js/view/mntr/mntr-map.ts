...
//declare var audiocomp: any;
declare var RJSON: any;
declare var REQUEST_CONTEXT_PATH;
declare var cmdReScheduleExecute;
declare var resetBatteryMarker;
declare var createSeqPlayerDevice;
declare var createReplay;
declare var regCollapseEventHandler;
declare var initOverlayMap;
declare var setOldSchedulingPath;
declare var setNewSchedulingPath;
declare var getEnvMap;
declare var setAgentMarkerInit;
...
var agentws = (function () {
  var wrap = { sendMessage: null as Function };
  ...
  ws.onmessage = function (e) {
    ...
    */
    ...
    if (pushLogJson) pushLogJson(parseData);
    ...
    if (parseData.msgId === "STAT") {
      ...
      if (appbody) appbody.removeAlarm(parseData.data.eventSn);
    } else if (parseData.msgId === "MQTTAGENTS") {
      // mqtt status 업데이트.
      // console.log(JSON.parse(parseData.data));
      // console.log("data", parseData.data);
      (parseData.data || []).forEach(function (ele: If.IfMqttStat) {
        ...
        if (location === ele.location) {
		  if (appbody) appbody.setHealthy(ele.agentId, ele.isHealthy);
        }
      ...
      if (appbody) appbody.ledkey = appbody.ledkey + 1;
      ...
      if (appbody) appbody.removeAlarmAll();
...
function marker_stat(data) {
  ...
  if (appbody) appbody.addAgentData(data);
} // Event 발생 시 호출됨
...
function marker_event(evt) {
  ...
  if (appbody) {
    ...
    if (info.info) image = info.info.image;
  }
  ...
  if (appbody) appbody.addEvent(evt);
...
function drawCallback(img) {
  var canvas = document.getElementById("playerBig") as HTMLCanvasElement;
...
$("#playerModal").on("show.bs.modal", function (e) {
  ...
  if (selagent > 9) {
	...
	if (appbody.isPlaying) $('#moveController').css('display', 'block');
...
$("#playerModal").on("hidden.bs.modal", function (e) {
  console.log("hidden");
  if (selplayer) selplayer.clearDrawCallback();
...
function moveAgent(area:string,lat:number,lng:number){
...
function moveup1() {	// mousedown, keydown function
  $('#moveStyleUp').removeClass('bi-arrow-up-circle-fill');
  $('#moveStyleUp').addClass('bi-arrow-up-circle');
  let value = parseFloat((moveControllerJson.lin + 0.1).toFixed(1));
...
$('#moveLinVelDown').mousedown(() => {
  movedown1();
});
$('#moveLinVelDown').mouseup(() => {
  movedown2();
});
...
function moveleft1() {	
  ...
  let value = parseFloat((moveControllerJson.ang - 0.1).toFixed(1));
  if (floatMinMaxCheck(value)) {
...
$('#moveAngVelLeft').mousedown(() => {
  moveleft1();
});
$('#moveAngVelLeft').mouseup(() => {
  moveleft2();
});
...
$('#moveAngVelRight').mousedown(() => {
  moveright1();
});
$('#moveAngVelRight').mouseup(() => {
  moveright2();
});
...
$('#moveLinAngReset').mousedown(() => {
  movereset1();
});
$('#moveLinAngReset').mouseup(() => {
  movereset2();
});
...
function clickSettings() {
  ...
  appset.getSettings();
  ($("#settingsModal") as any).modal("show");
...
function clickAgentAutoSearch() {
  ...
  ($("#appSearchModal") as any).modal("show");
...
function reStartDroneMon() {
  ...
  axios
    ...
    .then(function (res) {
      ...
      console.log("url", url);
      const quality = get_drone_quality(url);
...
function clickDroneMon() {
  ...
  ($("#droneMonModal") as any).modal("show");
...
function cmdApplyDroneQuality() {
  ...
  // 1. 데이터를 저장.
  const qual = parseInt("" + $("#droneQuality").val());
  ...
  const place = $("#droneQuality").data("place");
  let url = $("#droneQuality").data("url");
  console.log("url before ", url, ", qual", qual);
  let oldqual = parseInt(get_drone_quality(url));
  if (oldqual === qual) {
    alert("영상품질이 이전 설정과 동일합니다.");
    return;
  }
  url = set_drone_quality(url, qual);
  let param = { dronUrlPohang: "", dronUrlGwangju: "" };
  if (place === "ph") {
...
function get_drone_quality(url: string) {
  ...
  const kv = urls[1].split("&").find((ele: string) => ele.startsWith("quality"));
  if (kv) {
...
function set_drone_quality(url: string, val: number) {
  let newurl: string = "";
...
$(document).ready(function () {
  ...
  appbody = new Vue({
    ...
    mounted: function () {
      this.$refs.player.forEach((item, idx) => {
    ...
    methods: {
      ...
      getAreaName: function (evt) {
        if (evt.areaCode == "P") return "포항";
        if (evt.areaCode == "G") return "광주";
        return "";
      },
      getPos: function (v) {
        if (!v) return "";
      ...
      clickPlay: function () {
        if (this.isPlaying) this.stopPlayers();
        else this.startPlayers();
      },
      ...
      setupAgents: function (app, cdgrpCd) {
        ...
        axios
          ...
          .then(function (res) {
            ...
            var idx = 0;
            res.data.forEach(function (d: If.IfAgent) {
              d.info = RJSON.parse(d.codeDs);
              app.agents.push(d);
              oldAgents.push(d);
              var p = player[idx];
              ...
              if (p) {
                ...
                if (appbody.isPohang) p.areaCode = "P";
                else p.areaCode = "G";
      ...
      setHealthy: function (robotId, isHealthy) {
        ...
        app.agents
          ...
          .forEach(function (ele) {
            if (isHealthy) app.isHealthy.push(robotId);
      ...
      getAgentById: function (id) {
        ...
        let o = arr.find(function (ele) {
          if (ele.info && ele.info.agentId == id) return true;
          return false;
        });
        if (o) return o;
        return {};
      },
      getAgentCnt : function (onlyRobot:boolean) {
	    if (this.appSearchFixedBtn) {
		  return appsearch.agent_checked.length;
	    } else {
          if (onlyRobot) {
            return  this.agents.filter((agent) => agent.info.markerImage === "robot.png").length || 10;
          } else {
      ...
      cmdReScheduleAbnormal: function (evt: If.IfTbEvt) {
        console.log("cmdReScheduleAbnormal", evt);
        console.log("agent ", this.getAgentById(evt.robotId));
        let agent = this.getAgentById(evt.robotId);
        var app = this;
        let isFixed = agent?.info?.fixed === "Y" ? true : false;
        ...
        if (isFixed) app.doMoveToFixed("GOTO", evt.eventSn);
        else app.doMoveTo("GOTO", evt.eventSn);
        let url = "/monitoring/mqtt/scheduleabnormal/";
        if (isFixed) url = "/monitoring/mqtt/scheduleabnormalfixed/";
        axios
      ...
      doScheduleInit: function () {
        var app = this;
        if (getAreaNm() === "ph") app.saveInitAgentPh = [];
        else app.saveInitAgentGw = [];
        var arr = this.agents
          .filter(function (ele) {
            return ele.chkScheduleInit;
          })
          .map(function (ele) {
            if (getAreaNm() === "ph") app.saveInitAgentPh.push(ele.info.agentId);
            else app.saveInitAgentGw.push(ele.info.agentId);
            return "ids=" + ele.info.agentId;
          });
      ...
      doScheduleStop: function () {		// 로봇 긴급정지 명령
	    var app = this;
        var arr = this.agents
          .filter(function (ele) {
            return ele.chkScheduleInit;
      ...
      doScheduleReturn: function () {	// 로봇 복귀(충전대) 명령
	    var app = this;
        var arr = this.agents
          .filter(function (ele) {
            return ele.chkScheduleInit;
      ...
      getAgentIsHealthy: function (idx) {
        ...
        if (agent && agent.info) return agent.info.isHealthy;
        return false;
      },
      getData: function (idx) {
        ...
        if (agent.info) {
          var f = datas.find(function (e) {
            if (e.robotId == agent.info.agentId && e.areaCode == agent.info.areaCode) return true;
            return false;
          });
          if (f) return f;
        }
      ...
      addAgentData: function (data) {
        ...
        var idx = datas.findIndex(function (e) {
          if (e.robotId == data.robotId && e.areaCode == data.areaCode) return true;
          return false;
        });
      ...
      alarmReplay: function (evt) {
        ...
        axios
          ...
          .then(function (res) {
            ...
            ($("#eventReplayModal") as any).modal("show");
      ...
      removeMarkerAlarm: function (evt) {
        ...
        this.events.splice(
          this.events.findIndex(function (e) {
  ...
  appsearch = new Vue({
    ...
    data: {
      agentSettings: {},
      agentStatus: [] as Array<If.IfAgent>,
      isPohang: true,
      agent_checked: [],
      clickFixedBtn: false,
      maxRobotId: 9,
    ...
    methods: {
	  getAgentSettingList: function () {
		this.agentSettings.forEach((item) => {
		  const agent = item.info;
          const li = document.createElement("li");
          const type = (agent.fixed === "N" ? "이동형" : "고정형");
          const addValue = type + "(Agent ID : " + agent.agentId + ")";
      ...
      getAgentAutoSearch: function () {
		let areaCode = this.agentSettings.map((item) => item = item.info.areaCode);
        
        let status = this.agentStatus.data.slice();
        const loc = areaCode[0] === "P" ? "ph" : "gw";

        // 테스트 코드
        const healthyOk = [0, 2, 4, 6];
        healthyOk.forEach((ele) => {
        ...
        status.map((item, index) => {
          if (loc === item.location) {
            ...
            const type = (item.agentType === "ROBOT" ? "이동형" : "고정형");
            const addName = " " + type + "(Agent ID : " + item.agentId + ") : ";
            const addStatus = (item.isHealthy === true ? "정상" : "불량");
            const nameNode = document.createTextNode(addName);
            const statusNode = document.createTextNode(addStatus);
      ...
      clickFixed: function () {
        this.agent_checked = [];
	    let check = document.getElementById("getAgentStatus").getElementsByTagName("input");
        ...
        this.agent_checked.forEach((ele) => {
          if (ele <= this.maxRobotId) moveCount++;
        });

        appbody.numOfMovingAgent = moveCount;

        if (!appbody.appSearchFixedBtn) {
          this.agent_checked.forEach((item) => {
            appbody.agents.forEach((ele) => {
              if (Number(item) === ele.info.agentId) this.newAgents.push(ele);
            })
          });
          this.agent_checked.forEach((item) => {
            agentInfos.forEach((ele) => {
              if (Number(item) === ele.info.agentId) this.newAgentInfos.push(ele);
            })
          });
        } else {
          this.agent_checked.forEach((item) => {
            oldAgents.forEach((ele) => {
              if (Number(item) === ele.info.agentId) this.newAgents.push(ele);
            })
          });
          this.agent_checked.forEach((item) => {
            oldAgentInfos.forEach((ele) => {
              if (Number(item) === ele.info.agentId) this.newAgentInfos.push(ele);
            })
          });
...
function doClick(cdgrpCd) {
  ...
  $('input:radio[name="envmap"]').each(function (index) {
    ...
    (this as any).checked = false; //checked 처리
  });
...
function clickViewMileage() {
  ...
  ($("#mileageMonModal") as any).modal("show");
  appnav.btnMileageClicked = true;
  const loc = appbody.isPohang ? "ph" : "gw";
  setMileageMap(loc);
  $('#locationMileage').val(loc).prop("selected", true);
  appbody.agentStats.map((item) => {
...
function drawViewMileage(agents) {
  ...
  let chartData = [];
  let agentInfo = { id: [], color: [] };
  $('#chart_mileage').empty();

  let colors = [
    "#4B0082", // Robot_1 Color(Indigo)
    "#008000", // Robot_2 Color(Green)
    "#0000FF", // Robot_3 Color(Blue)
    "#1E90FF", // Robot_4 Color(DodgerBlue)
    "#00CED1", // Robot_5 Color(DarkTurquoise)
    "#E9967A", // Robot_6 Color(DarkSalmon)
    "#FF8C00", // Robot_7 Color(DarkOrange)
    "#FF7F50", // Robot_8 Color(Coral)
    "#FF69B4"  // Robot_9 Color(HotPink)
  ];

  const data = appbody.dataMileage;
  if (agents.length > 0) {
    ...
    agents.map((item, idx) => {
	  if (idx === 0) chartData.push(["RobotID", "Mileage", { role: "annotation" }, { role: "style" }]);
  ...
  let drawData = google.visualization.arrayToDataTable(chartData);
  const col = drawData.getColumnRange(1);
  console.log("column range min/max ===> ", col.min, col.max);
  let _max = Math.ceil(col.max);
  let options = {
    width: 600,
    hAxis: {
      title: "마일리지(Km)",
...
async function pathDraw(id, color) {
  console.log("pathDraw ===> ", id, color);
  const start = $('#mileageStartDate').val();
  const end = $('#mileageEndDate').val();

  const res = await axios.get("/monitoring/mntr/mileagerouter?id=" + id + "&" + "start=" + start + "&" + "end=" + end);
...
function addMileageRouter(id, paths, color) {
  ...
  $('#map_loading').empty();
  mileageRouter.forEach((item) => item.setMap(map_mileage));
}

// 이동 경로 삭제 - mileage path remove
function removeMileageRouter() {
  mileageRouter.forEach((item) => item.setMap(null));
  mileageRouter = [];
}

$('input[name="mileageDateIn"]').on("change", function() {
  let oldStartDt, oldEndDt;
  let mileageStartDt = $('#mileageStartDate').val();
  let mileageEndDt = $('#mileageEndDate').val();
...
function mileageAgentCount() {
  let agents = [];
  $('#selectAgentList').find('input').map((idx, ele) => {
    if (idx !== 0 && ele.checked) agents.push(ele.value);
  })
  return agents;
}
...
function setChartLayout(cmd) {
  if (cmd === "chart-loading") {
	$('#chart_mileage').empty();
    $('#chart_mileage').append(
      '<img src="/monitoring/resources/images/loading/loading1.gif" style="padding-left: 140px; padding-top: 100px;">'
    );
  } else if (cmd === "map-loading") {
	console.log("map_loading");
	$('#map_loading').append(
      '<img src="/monitoring/resources/images/loading/loading3.gif" style="padding-left: 140px; padding-top: 100px;">'
    );
...
function clickViewScheduling() {
  ...
  if (!appnav.btnSchedulingClicked) {
    appbody.agents.filter((ele) => {
	  if (ele.chkScheduleInit) ele.chkScheduleInit = false;
	})
  }
}
...
hdgSlider.on("input", function () {
  $("#hdgVal").text(hdgSlider.val() as string);
});

var pitchSlider = $("#pitch");
pitchSlider.on("input", function () {
  $("#pitchVal").text(pitchSlider.val() as string);
});

var yawSlider = $("#yaw");
yawSlider.on("input", function () {
  $("#yawVal").text(yawSlider.val() as string);
});
