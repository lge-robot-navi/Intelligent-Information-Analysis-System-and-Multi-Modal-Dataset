...
function setAgentMarkerInit(data) {
    ...
    data.forEach(function (d) {
        ...
        d.marker = new MarkerWithLabel({
            ...
            labelAnchor: new google.maps.Point(35, 75),
            labelClass: cssLabel,
        });
        d.marker.setMap(map);
        // robot 아이콘만 배터리 표시
...
var moveCursor = { flag: false, id: "" };
function getLabelContent(data) {
    moveCursor.flag = false;
    var content = document.createElement("div");
    var agentBtn = createImgBtn(data);
    ...
    var agentNamePanel = document.createElement("p");
    ...
    if (data.info.markerImage === "drone.png" || data.info.markerImage === "robot.png") {
        ...
        var batteryPanel = createBatteryPanel(data);
...
function createBatteryPanel(data) {
    var batteryPanel = document.createElement("label");
...
function moveBtnEvt(id) {
    moveCursor.id = id;
    $("#contextmenu-" + id).css("display", "none");
    setMoveAgentCursor(true);
} //move
function setMoveAgentCursor(flag) {
...
//# sourceMappingURL=mntr-agent.js.map