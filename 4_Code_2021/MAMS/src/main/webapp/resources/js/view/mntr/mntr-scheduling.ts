...
declare var _: typeof mlodash; /////////////////////////////////////////////////// mntr-scheduling.js (로봇 스케쥴링 처리)
...
function schedulingRouter(id) {
  if ($("#robot_old_" + id).is(":checked")) {
    ...
    if (robotRouter[id] === undefined) addRouter(id, oldSchedulingPath[id], "#005ef8");
  } else {
    console.log("여기===>remove", id);
    if (robotRouter[id] !== undefined) removeRouter(id);
  }
}
...
function reSchedulingRouter(id) {
  if ($("#robot_new_" + id).is(":checked")) {
    console.log("여기===>add", id);
    if (robotReRouter[id] === undefined) addReRouter(id, newSchedulingPath[id], "#ff0000");
  } else {
    console.log("여기===>remove", id);
    if (robotReRouter[id] !== undefined) removeReRouter(id);
  }
}
...
function schedulingRouterAll() {
  if ($("#robot_old_all").is(":checked")) {
    $('input:checkbox[name="robot_old_id"]').each((idx, item) => {
	  const $id = $(item).data("agent_id");
      console.log("[schedulingRouterAll] agent id : ", $id);
      (item as any).checked = true; //checked 처리
      schedulingRouter($id);
    });
  } else {
    $('input:checkbox[name="robot_old_id"]').each((idx, item) => {
	  const $id = $(item).data("agent_id");
      console.log("[schedulingRouterAll] agent id : ", $id);
      (item as any).checked = false; //checked 처리
      schedulingRouter($id);
    });
  }
}
...
function reSchedulingRouterAll() {
  if ($("#robot_new_all").is(":checked")) {
    $('input:checkbox[name="robot_new_id"]').each((idx, item) => {
	  const $id = $(item).data("agent_id");
