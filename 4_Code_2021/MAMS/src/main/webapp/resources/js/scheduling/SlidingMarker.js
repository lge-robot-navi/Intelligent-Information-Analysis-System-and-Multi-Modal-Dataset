...
(function (root, factory) {
  ...
  var animateTo,
    markerAnimate_AnimateTo_Wrapper = function (destPosition, easing, duration) {
      //default implementation based on markerAnimate
  ...
  var decorates = function (childCtor, parentCtor) {
    ...
    $.extend(childCtor.prototype, {
      ...
      _setInstancePositionAnimated: function (position) {
        ...
        if (that._constructing) {
          //pass by
          return;
        }

        if (!position || !that._instance.getPosition()) {
          //If position is set for a first time, no animation should be applied
